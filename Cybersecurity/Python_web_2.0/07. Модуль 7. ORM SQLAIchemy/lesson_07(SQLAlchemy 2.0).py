...
if __name__ == "__main__":
    ...
    stmt = (
        select(User)
        .where(User.fullname.like("%ha%"))
        .where(User.fullname != "Shaun Beck")
    )
    result = session.execute(stmt)
    ...
    for user in users:  # створюємо запис Post для кожного користувача
        post = Post(
            title=f"Title {user[1]}", body=f"Body post user {user[1]}", user_id=user[0]
        )
        session.add(post)
    session.commit()
    ...
    stmt = (
        select(
            User.fullname, func.count(Post.id)
        )  # створюємо об'єкт select із вибіркою імені користувача та кількості постів
    ...
    for name, count in results:  # перебираємо результати
        print(
            f"{name} has {count} posts"
        )  # виводимо ім'я користувача та кількість постів
"""
# =================================================================================================
